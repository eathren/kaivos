...
@export var speed: float = GameState.get_trawler_speed()
...
func _update_direction_line(delta: float) -> void:
	...
	if v_len < 1.0:
		...
		var speed_ratio: float = clamp(v_len / max_speed_for_line, 0.0, 1.0)
		var target_length: float = max_line_length * speed_ratio
	...
	var end: Vector2 = dir * current_line_length
